<?php
	...
	function Reminder($message, $args) {
		...
		if (empty($args) || !is_numeric(intval($args2[0])) || intval($args2[0]) < 1 || !preg_match('/(min(?:ute)?|hour|day|week|month)s?/',$args2[1])) {
			return simpleEmbed("Chat Reminders", "attachment://bot.webp", null, "Invalid syntax used. Try *!remindme 5 mins/hours/days [message]*", $message, true); 
		}
		...
		if ($time > (time() + 2592000*12)) { 
			return simpleEmbed("Chat Reminders", "attachment://bot.webp", null, "The time period provided is too far into the future. Limit it to under a year.", $message, true); 	
		}
		...
		if ($result->num_rows > 4) {
			return simpleEmbed("Chat Reminders", "attachment://bot.webp", null, "Cannot set a new reminder for you as you already have 5 set.", $message, true); 
		}
