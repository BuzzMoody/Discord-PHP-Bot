			...
			$http->get('https://api.formula1.com/v1/event-tracker', $headers)->then(
				function (ResponseInterface $response) use ($message) {
					...
					foreach ($output->seasonContext->timetables as $event) {
						$fieldval = ($event->state == 'completed') ? "~~".$utils->toAusTime($event->startTime, 'G:i', null, $event->gmtOffset)." - ".$utils->toAusTime($event->endTime, 'G:i', null, $event->gmtOffset)."~~ - [Results](https://www.formula1.com/en/results/{$output->seasonContext->seasonYear}/races/{$output->fomRaceId}/F1/".str_replace(' ', '/', strtolower($event->description)).")" : $utils->toAusTime($event->startTime, 'G:i', null, $event->gmtOffset)." - ".$utils->toAusTime($event->endTime, 'G:i', null, $event->gmtOffset)." (Starts <t:".strtotime($utils->toAusTime($event->startTime, 'Y-m-d\TH:i:s', null, $event->gmtOffset, true)).":R>)";
