<?php
	...
	class Dota {
		...
		public function __construct(private Discord\Discord $discord, private PDO $pdo, private BotUtils $utils) { }
			...
			if ($current_hour >= 7 || $current_hour <= 1) {
				...
				\React\Promise\all($promises)->then(function (array $results) {
					...
					foreach ($newMatches as $matchID => $playersInMatch) {
						...
						$this->postToDiscord((int) $matchID, $playersInMatch);
		...
		private function saveMatch(int $steamID, int $matchID): void {
		...
		private function calcLevel(int $xpm, int $duration): int {
